...
The `runtime.user` object has the same fields as your `@auth.authenticate` handler
returns: `identity`, `display_name`, `permissions`, and any custom fields (accessible
via `user["role"]`, `user["team_id"]`, etc.).

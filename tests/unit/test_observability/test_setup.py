...
from src.agent_server.observability.setup import setup_observability
...
class TestSetupObservability:
    ...
    def mock_deps(self):
        ...
        with (
            patch(
                "src.agent_server.observability.setup.get_observability_manager"
            ) as mock_get_manager,
            patch(
                "src.agent_server.observability.setup.otel_provider"
            ) as mock_otel_provider,
            patch("src.agent_server.observability.setup.logger") as mock_logger,
        ):
    ...
    def test_setup_registers_provider_always(self, mock_deps):
        ...
        # Assert
        mock_deps["manager"].register_provider.assert_called_once_with(
            mock_deps["provider"]
        )
    ...
    def test_setup_initializes_when_enabled(self, mock_deps):
        ...
        mock_deps["logger"].info.assert_called_with(
            "Observability subsystem initialized successfully."
        )
    ...
    def test_setup_skips_initialization_when_disabled(self, mock_deps):
        ...
        mock_deps["logger"].info.assert_called_with(
            "Observability is disabled (no targets configured)."
        )
    ...
    def test_setup_handles_exceptions_gracefully(self, mock_deps):
        ...
        assert (
            ...
            in mock_deps["logger"].error.call_args[0][0]
        )

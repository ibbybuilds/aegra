...
The `context` field is only available on `_ExecutionRuntime` (i.e., during `threads.create_run`). For read/introspection access contexts, `runtime` is a `_ReadRuntime` which has no `context` field — use `runtime.execution_runtime` to narrow.

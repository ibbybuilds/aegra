...
from agent_server.core.accountability_orm import ActionItem, Notification
        ...
        try:
            ...
            async with session_maker() as session:
                ...
                cutoff_read = now - timedelta(
                    days=7
                )  # Keep read notifications for 7 days
                cutoff_old = now - timedelta(
                    days=30
            ...
            async with session_maker() as session:
                ...
                result = await session.execute(
                    select(ActionItem).where(
                        and_(ActionItem.status == "pending", ActionItem.due_date < now)
                    )
                )
                ...
                for item in overdue_items:
                    ...
                    if not exists_query.scalars().first():
                        ...
                        logger.info(
                            "Generated overdue notification", action_item_id=item.id
                        )

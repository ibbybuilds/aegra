...
When `ENABLE_PROMETHEUS_METRICS=true`, Aegra registers 21 application-level metrics covering the worker executor, lease reaper, and run lifecycle. These are separate from the HTTP-level metrics provided by `prometheus-fastapi-instrumentator`. Redis-specific metrics (e.g. `queue_depth`, `redis_reachable`, `dequeue_errors`) remain zero-valued when Redis is disabled.
...
- `dispatched` — run submitted to queue (`submit()`)
- `completed` — run reached terminal state via `execute_run` or cancel endpoint (status: success/error/interrupted)
...
| `aegra_runs_completed_total` | Counter | `graph_id`, `status` | Runs completed (success/error/interrupted) |
...
| `aegra_redis_reachable` | Gauge | — | 1 if Redis responded to last BLPOP, 0 otherwise |
...
| `aegra_lease_losses_total` | Counter | `graph_id` | Lease lost during execution |
...
- **Execution**: 0.5, 1, 2, 5, 10, 30, 60, 120, 300, 600s
- **Queue wait**: 0.1, 0.5, 1, 2, 5, 10, 30, 60, 120, 300s
- **Reaper cycle**: 0.01, 0.05, 0.1, 0.25, 0.5, 1, 2, 5s

### CancelledError discrimination

Worker runs can be cancelled for three reasons, each handled differently:

1. **User cancel** — API calls `mark_user_cancellation(run_id)` before `task.cancel()`. The run is finalized as `interrupted` and counted in `completed`.
2. **Lease-loss** — heartbeat detects another worker claimed the run. Finalization is skipped; the reaper re-enqueues. Temporary gap in the identity invariant (self-heals).
3. **Timeout** (default) — `asyncio.wait_for` fires. `execute_run` skips finalization; the outer `_execute_and_release` handler finalizes as `error` and increments `run_timeouts`.

### Known gaps

- **Dev mode**: Redis-specific metrics (`queue_depth`, `redis_reachable`, `dequeue_errors`, reaper counters) are always zero. The identity invariant does not hold — `dispatched` is not incremented because `LocalExecutor` bypasses the queue.
...
- **`redis_reachable=0`** does not mean the system is down — Postgres fallback polling may be active.

...
class ActionItem(Base):
    ...
    status: Mapped[str] = mapped_column(Text, server_default=text("'pending'")) # pending, in_progress, completed, skipped
    due_date: Mapped[datetime | None] = mapped_column(
        TIMESTAMP(timezone=True), nullable=True
    )
    priority: Mapped[str] = mapped_column(Text, server_default=text("'normal'")) # critical, urgent, normal, low
    category: Mapped[str | None] = mapped_column(Text, nullable=True) # e.g. "Portfolio", "Learning"
...
class Notification(Base):
    ...
    metadata_json: Mapped[dict] = mapped_column(
        "metadata", JSONB, server_default=text("'{}'::jsonb") # action_item_id, etc.
    )

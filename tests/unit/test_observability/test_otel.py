...
from src.agent_server.observability.targets import (
    BaseOtelTarget,
    GenericOtelTarget,
    LangfuseTarget,
    PhoenixTarget,
...
class TestOpenTelemetryProviderInit:
    ...
    def test_init_disabled_by_default(self):
        ...
        with patch("src.agent_server.observability.otel.settings") as mock_settings:
    ...
    def test_init_ignores_unknown_targets(self):
        ...
        with patch("src.agent_server.observability.otel.settings") as mock_settings:
            ...
            with patch("src.agent_server.observability.otel.logger") as mock_logger:
                provider = OpenTelemetryProvider()
                ...
                mock_logger.warning.assert_called_with(
                    "Unknown OTEL target in settings: UNKNOWN_VENDOR"
                )
...
class TestOpenTelemetryProviderSetup:
    ...
    def mock_deps(self):
        ...
        with (
            patch("src.agent_server.observability.otel.TracerProvider") as mock_tp,
            patch("src.agent_server.observability.otel.BatchSpanProcessor") as mock_bsp,
            patch(
                "src.agent_server.observability.otel.ConsoleSpanExporter"
            ) as mock_cse,
            patch(
                "src.agent_server.observability.otel.LangChainInstrumentor"
            ) as mock_lci,
            patch("src.agent_server.observability.otel.trace") as mock_trace,
            patch("src.agent_server.observability.otel.Resource") as mock_resource,
            patch("src.agent_server.observability.otel.settings") as mock_settings,
        ):
            # Setup defaults
            mock_settings.observability.OTEL_SERVICE_NAME = "test-service"
            mock_settings.observability.OTEL_CONSOLE_EXPORT = (
                False  # Default to False to prevent noise
            )
    ...
    def test_setup_creates_correct_resource(self, mock_deps):
        ...
        mock_deps["tp"].assert_called_with(
            resource=mock_deps["resource"].create.return_value
        )
    ...
    def test_setup_instruments_globally(self, mock_deps):
        ...
        mock_deps["trace"].set_tracer_provider.assert_called_with(
            mock_deps["tp"].return_value
        )

        mock_deps["lci"].return_value.instrument.assert_called_with(
            tracer_provider=mock_deps["tp"].return_value
        )
    ...
    def test_get_metadata_returns_correct_structure(self):
        ...
        with patch("src.agent_server.observability.otel.settings") as mock_settings:
            ...
            meta = provider.get_metadata(
                run_id="run-123", thread_id="thread-456", user_identity="user-789"
            )

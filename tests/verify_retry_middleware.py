...
async def test_forced_retry_logic():
    ...
    def override(**kwargs):
        ...
        if "system_message" in kwargs:
            print(f"  [PASS] System message updated! New content length: {len(kwargs['system_message'].content)}")
            if "RETRY SILENTLY" in kwargs['system_message'].content or "DO NOT output any text" in kwargs['system_message'].content:

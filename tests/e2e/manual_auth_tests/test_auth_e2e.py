...
class TestCoreRoutesAuth:
    ...
    def test_core_routes_accept_with_valid_token(self):
        ...
        assert response.status_code != 401, (
            f"Expected not 401, got {response.status_code}: {response.text}"
        )
...
class TestCustomRoutesAuth:
    ...
    def test_custom_protected_requires_auth(self):
        ...
        assert response_no_auth.status_code == 401, (
            f"Expected 401 without auth, got {response_no_auth.status_code}"
        )
        ...
        assert response_with_auth.status_code == 200, (
            f"Expected 200 with auth, got {response_with_auth.status_code}"
        )
...
class TestCustomRouteAuthConfig:
    ...
    def test_custom_public_endpoint_behavior(self):
        ...
        assert response_no_auth.status_code in (200, 401), (
            f"Unexpected status {response_no_auth.status_code}"
        )

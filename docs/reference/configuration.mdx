...
```json
{
  "graphs": {
    ...
    "agent_js": {
      "runtime": "langgraphjs",
      "path": "./src/agent-js/graph.ts:graph"
    }
  }
}
```
...
| Value | `string \| object` | Graph definition (Python string path or runtime object) |

### Graph value formats

`graphs.<graph_id>` supports either format:

1. Legacy string format (Python):
...
|-------|------|----------|-------------|
| `runtime` | `"python" \| "langgraphjs"` | No | Graph runtime. Defaults to `"python"` when omitted. |
| `path` | `string` | Yes | Export path in format `./path/to/file.ext:export_name` |

For `langgraphjs` graphs, Aegra automatically manages a Node.js bridge subprocess. Requires Node.js 18+ installed on the server. Run `npm install` in your graph's directory to install JavaScript dependencies. See the [LangGraph.js guide](/guides/langgraphjs) for details.
